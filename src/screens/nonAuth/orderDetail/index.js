...
const OrderClientDetail = (props) => {
    ...
    const getOrderDetail = (order_id) => {
        ...
        let config = {
            ...
            endPoint: '/api/providerOrderDetail',
            type: 'post'
        }
...
const CardClientInfo = ({ data }) => {
    ...
    return (<Card containerStyle={{ borderRadius: 10 }}>
        <View style={{ flexDirection: "row" }}>
            <View style={{ flex: 1, flexDirection: "row" }}>
                <Avatar
                    size="large"
                    rounded
                    source={data?.customers_image ? { uri: BASE_URL + data?.customers_image } : placeholder_image}
                />
                <View style={{ marginLeft: 10, justifyContent: "center" }}>
                    <Text style={[styles.greenTextStyle, { fontSize: 16 }]}>{data?.customers_first_name}</Text>
